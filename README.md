...
1. **ボール操作中〜その後（layer 1）** → 常にクリック  
   ボールを動かすと layer 1 が乗り、同じ位置は素の `&mkp MB1` / `&mkp MB2`。直前の文章操作による句読点クールタイムはここで打ち切る。クリックキー以外を押すか、ボールを止めて 1.5s で layer 1 が外れる。外れたあとにまた文字を打てば、そこから新しいクールタイムが始まる。
2. **直前 1.5 秒が文章操作（layer 1 でない）** → 句読点（`,` / `.`）
3. **それ以外（アイドル、Ctrl+S など）** → クリック
